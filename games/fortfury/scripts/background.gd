...
func _draw_grassland(vw: float, vh: float) -> void:
	...
	draw_rect(Rect2(0, 0, vw, vh * 0.25), Color(0.28, 0.52, 0.88))
	draw_rect(Rect2(0, vh * 0.25, vw, vh * 0.20), Color(0.38, 0.65, 0.92))
	draw_rect(Rect2(0, vh * 0.45, vw, vh * 0.20), Color(0.55, 0.78, 0.96))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.26, 0.55, 0.18))
	...
	# Distant haze
	draw_rect(Rect2(0, vh * 0.58, vw, 28), Color(0.80, 0.90, 1.0, 0.22))
	...
	var hill: PackedVector2Array = [Vector2(0, vh * 0.68)]
	for i in 22:
		var x := vw * i / 21.0
		var y := vh * 0.68 + sin(x * 0.0028) * 55 + cos(x * 0.0065 + 0.8) * 28
		hill.append(Vector2(x, y))
	hill.append(Vector2(vw, vh))
	hill.append(Vector2(0, vh))
	draw_colored_polygon(hill, Color(0.22, 0.50, 0.14))

	# Ground texture strips
	draw_rect(Rect2(0, vh * 0.70, vw, 8), Color(0.30, 0.58, 0.20, 0.55))
	draw_rect(Rect2(0, vh * 0.76, vw, 5), Color(0.18, 0.42, 0.12, 0.35))

func _draw_village(vw: float, vh: float) -> void:
	# Warm overcast sky
	draw_rect(Rect2(0, 0, vw, vh * 0.30), Color(0.45, 0.58, 0.78))
	draw_rect(Rect2(0, vh * 0.30, vw, vh * 0.35), Color(0.60, 0.72, 0.88))
	...
	# Village silhouettes — varied buildings
	...
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.28, 0.52, 0.20))
	draw_rect(Rect2(0, vh * 0.67, vw, 6), Color(0.35, 0.60, 0.24, 0.60))

func _draw_forest(vw: float, vh: float) -> void:
	# Deep forest sky
	draw_rect(Rect2(0, 0, vw, vh * 0.45), Color(0.20, 0.42, 0.22))
	draw_rect(Rect2(0, vh * 0.45, vw, vh * 0.20), Color(0.28, 0.55, 0.25))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.14, 0.36, 0.10))

	# Background tree canopy
	for i in 20:
		var tx := vw * i / 19.0 + fmod(float(i) * 31.7, 35.0) - 17.0
		var th := 120.0 + fmod(float(i) * 53.2, 90.0)
		var tw := 55.0 + fmod(float(i) * 21.1, 35.0)
		_draw_tree_silhouette(tx, vh * 0.65, th, Color(0.08, 0.30, 0.08, 0.85))

	# Foreground trees (larger, darker)
	for i in 9:
		var tx := vw * 0.05 + i * (vw * 0.11)
		var th := 160.0 + fmod(float(i) * 41.9, 60.0)
		var tw := 65.0 + fmod(float(i) * 19.3, 30.0)
		_draw_tree_silhouette(tx, vh * 0.65, th, Color(0.06, 0.22, 0.06, 0.95))

	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.10, 0.30, 0.08))

func _draw_desert(vw: float, vh: float) -> void:
	# Hot layered sky
	draw_rect(Rect2(0, 0, vw, vh * 0.22), Color(0.85, 0.42, 0.10))
	draw_rect(Rect2(0, vh * 0.22, vw, vh * 0.22), Color(0.96, 0.62, 0.18))
	draw_rect(Rect2(0, vh * 0.44, vw, vh * 0.21), Color(1.0, 0.82, 0.40))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.88, 0.75, 0.45))
	...
	# Heat shimmer band
	draw_rect(Rect2(0, vh * 0.60, vw, 20), Color(1.0, 0.90, 0.65, 0.30))

	# Sand dunes
	var dune1: PackedVector2Array = [Vector2(0, vh * 0.72)]
	for i in 28:
		var x := vw * i / 27.0
		var y := vh * 0.70 + sin(x * 0.0018) * 70 + sin(x * 0.0048 + 1.2) * 38
		dune1.append(Vector2(x, y))
	dune1.append(Vector2(vw, vh))
	dune1.append(Vector2(0, vh))
	draw_colored_polygon(dune1, Color(0.80, 0.65, 0.32))

	# Lighter sand highlight ridge
	var ridge: PackedVector2Array = [Vector2(0, vh * 0.69)]
	for i in 28:
		var x := vw * i / 27.0
		var y := vh * 0.67 + sin(x * 0.0018) * 70 + sin(x * 0.0048 + 1.2) * 38
		ridge.append(Vector2(x, y))
	ridge.append(Vector2(vw, vh * 0.75))
	draw_colored_polygon(ridge, Color(0.92, 0.78, 0.48, 0.5))

func _draw_mountain(vw: float, vh: float) -> void:
	# Cool atmospheric sky
	draw_rect(Rect2(0, 0, vw, vh * 0.30), Color(0.22, 0.35, 0.58))
	draw_rect(Rect2(0, vh * 0.30, vw, vh * 0.35), Color(0.38, 0.52, 0.72))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.30, 0.35, 0.28))

	# Far mountains (lighter, hazy)
	for i in 4:
	...
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.28, 0.32, 0.26))

func _draw_arctic(vw: float, vh: float) -> void:
	# Pale arctic sky with aurora
	draw_rect(Rect2(0, 0, vw, vh * 0.50), Color(0.60, 0.80, 0.96))
	draw_rect(Rect2(0, vh * 0.50, vw, vh * 0.15), Color(0.72, 0.88, 1.0))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.85, 0.93, 1.0))
	...
	# Snowflakes (static, deterministic)
	for i in 55:
	...
	# Ice ground with reflection
	var ice_col := Color(0.70, 0.88, 1.0, 0.85)
	draw_rect(Rect2(0, vh * 0.62, vw, 22), ice_col)
	draw_rect(Rect2(0, vh * 0.62, vw, 4), Color(1, 1, 1, 0.55))
	# Ice cracks
	for i in 7:
...
func _draw_apocalypse(vw: float, vh: float) -> void:
	# Ashen dark sky
	draw_rect(Rect2(0, 0, vw, vh * 0.35), Color(0.08, 0.05, 0.04))
	draw_rect(Rect2(0, vh * 0.35, vw, vh * 0.30), Color(0.18, 0.10, 0.06))
	draw_rect(Rect2(0, vh * 0.65, vw, vh * 0.35), Color(0.18, 0.10, 0.06))
	...
	draw_rect(Rect2(0, vh * 0.64, vw, 8), Color(0.35, 0.10, 0.02, 0.65))
