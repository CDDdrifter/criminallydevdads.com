...
func _draw() -> void:
	...
	# Dots are largest and most opaque at the launch point, fading toward impact
	for i in range(0, count - 1, 2):
		var frac: float = 1.0 - float(i) / float(count - 1)
	...
	# Launch marker — distinct ring at the firing point
	var first: Vector2 = _points[0]
	draw_circle(first, 10.0, Color(0.55, 0.82, 1.0, 0.20))
	draw_circle(first, 6.5,  Color(0.85, 0.95, 1.0, 0.42))
	draw_circle(first, 3.5,  Color(1.00, 1.00, 1.00, 0.65))
	draw_arc(first, 13.0, 0.0, TAU, 32, Color(0.62, 0.88, 1.0, 0.35), 1.5)
